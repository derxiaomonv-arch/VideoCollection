...
(function () {
	...
	async function fetchDetail(apiAk, linkUrl) {
		// 第一步：通过本地代理获取 aweme_id
		const awemeIdRes = await fetch('/api/get_aweme_id', {
			method: 'POST',
			headers: {
		...
		if (!awemeIdRes.ok) throw new Error('获取aweme_id失败');
		const awemeIdData = await awemeIdRes.json();
		
		if (!awemeIdData || awemeIdData.code !== 200 || !awemeIdData.data) {
			throw new Error('无法获取有效的aweme_id');
		}
		
		// API返回的数据格式：{ code: 200, data: "aweme_id_string" }
		const awemeId = awemeIdData.data;
		
		// 第二步：通过本地代理使用 aweme_id 获取视频信息
		const videoRes = await fetch(`/api/fetch_video?aweme_id=${encodeURIComponent(awemeId)}`, { 
			method: 'GET' 
		});
		...
		if (!videoData || videoData.code !== 200 || !videoData.data || !videoData.data.aweme_detail) {
			throw new Error('无法获取有效的视频信息');
		}
		...
		// 转换为原有格式以保持兼容性
		// 封面URL从 origin_cover.url_list[0] 获取
		const coverUrl = (videoInfo.origin_cover && videoInfo.origin_cover.url_list && videoInfo.origin_cover.url_list.length > 0) 
			? videoInfo.origin_cover.url_list[0] 
			: (awemeDetail.cover_url || '');
		
		return {
			title: awemeDetail.desc || '无标题',
			url: videoInfo.download_addr.url_list[0] || '',
			cover: coverUrl,
			success: true
		};
	}
